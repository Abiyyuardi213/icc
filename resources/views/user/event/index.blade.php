...
<div class="mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('event.list') }}" class="px-4 py-2 bg-[#EC46A4] hover:bg-[#d63f93] text-white font-semibold rounded-lg shadow-md transition text-sm">
        Tambah Event
    </a>
</div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between pb-2">
                            ...
                            <span class="font-medium text-gray-800">{{ $team->leader_name ?? auth()->user()->name }}</span>
            ...
            <div class="p-6 border-t border-gray-100">
                <h4 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#EC46A4]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if($team->status === 'verified')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($team->event->tasks as $task)
                                @php
                                    ...
                                    $statusColor = $submission ? 'text-green-600 bg-green-50' : 'text-yellow-600 bg-yellow-50';
                                ...
                                <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                                    ...
                                    <p class="text-sm text-gray-500 mb-3 line-clamp-2">{{ Str::limit(strip_tags($task->description), 80) }}</p>
                                    <div class="flex justify-between items-center text-xs text-gray-500 mb-4">
                                        <span>Deadline: {{ $task->end_time->format('d M H:i') }}</span>
                                    </div>
                                    <a href="{{ route('user.tasks.show', $task->id) }}" class="block text-center w-full py-2 rounded-lg bg-gray-50 text-gray-700 hover:bg-[#EC46A4] hover:text-white font-medium transition text-sm">
                    ...
                    @endif
                @else
                    <div class="text-center py-6 bg-yellow-50 rounded-lg border border-yellow-100 text-yellow-700 text-sm">
