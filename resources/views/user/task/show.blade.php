...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
            ...
            @if ($task->file_path)
                <div class="mt-6 pt-6 border-t border-gray-100">
                    ...
                    <a href="{{ asset('storage/' . $task->file_path) }}" target="_blank"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-lg border border-gray-200 transition">
        ...
        @if ($submission && $submission->score !== null)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="font-bold text-lg text-gray-800 mb-4">Hasil Penilaian</h3>
                <div class="grid grid-cols-3 gap-4 text-center">
                    <div class="bg-green-50 p-4 rounded-lg border border-green-100">
                        <p class="text-sm text-gray-500 mb-1">Benar</p>
                        <p class="text-2xl font-bold text-green-600">{{ $submission->correct_answers ?? '-' }}</p>
                    </div>
                    <div class="bg-red-50 p-4 rounded-lg border border-red-100">
                        <p class="text-sm text-gray-500 mb-1">Salah</p>
                        <p class="text-2xl font-bold text-red-600">{{ $submission->wrong_answers ?? '-' }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form id="submissionForm" action="{{ route('user.tasks.store', $task->id) }}" method="POST"
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title"
                            value="{{ old('title', $submission->title ?? 'Submission for ' . $task->title) }}"
                    ...
                    <div>
                        ...
                        <input type="url" name="link_repository"
                            value="{{ old('link_repository', $submission->link_repository ?? '') }}"
                        ...
                        @if ($submission && $submission->file_path)
                            <div class="mb-3">
                                <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank"
                                    class="inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 text-sm font-medium rounded-md border border-green-200 transition">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition">{{ old('notes', $submission->notes ?? '') }}</textarea>
                    </div>

                    <div class="pt-4 border-t border-gray-100 flex justify-end">
                        <button type="submit" id="submitBtn"
                            class="bg-[#EC46A4] hover:bg-[#d63f93] text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
                ...
                <div class="space-y-3">
                    <div class="space-y-4">
                        @foreach ($submission->histories as $history)
                            <div
                                class="flex items-start gap-3 text-sm pb-3 border-b border-gray-50 last:border-0 last:pb-0">
                                <div
