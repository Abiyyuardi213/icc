    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            @if ($upcomingEvent)
                <div class="border border-pink-100 bg-pink-50 rounded-lg p-5">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span
                            class="text-xs bg-white text-[#EC46A4] px-2 py-1 rounded border border-pink-100 font-medium">Segera
                            Datang</span>
                    </div>
                    ...
                    <div class="mt-3 space-y-2">
                        @if ($upcomingEvent->preliminary_date)
                            <div class="flex items-center gap-2 text-sm">
                                <span class="inline-block w-2 h-2 rounded-full bg-yellow-400"></span>
                                <span class="font-medium text-gray-700">Penyisihan:</span>
                                <span class="text-gray-600">{{ $upcomingEvent->preliminary_date->format('d M Y') }}</span>
                            </div>
                        @endif
                        @if ($upcomingEvent->final_date)
                            <div class="flex items-center gap-2 text-sm">
                                <span class="inline-block w-2 h-2 rounded-full bg-purple-500"></span>
                                <span class="font-medium text-gray-700">Final:</span>
                                <span class="text-gray-600">{{ $upcomingEvent->final_date->format('d M Y') }}</span>
                            </div>
                        @endif
                    </div>
                    <div class="mt-4">
                        <a href="{{ route('user.events.index') }}"
                            class="text-sm font-semibold text-[#EC46A4] hover:underline">Lihat Detail Event &rarr;</a>
                    </div>
                </div>
            @else
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('user.events.index') }}"
                    class="text-sm text-[#EC46A4] hover:text-[#d63f93] font-medium">Lihat Semua</a>
            </div>
            <div class="space-y-4">
                ...
                @if ($teams->count() == 0)
                    <a href="{{ route('event.list') }}"
                        class="block text-center mt-4 w-full py-2 bg-[#EC46A4] text-white rounded-lg hover:bg-[#d63f93] text-sm font-medium transition">
                        Cari Event & Daftar
                    </a>
                @endif
            </div>
