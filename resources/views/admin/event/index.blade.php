...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        ...
        <button onclick="openCreateModal()"
            class="bg-[#EC46A4] hover:bg-[#d63f93] text-white font-medium py-2 px-4 rounded-lg flex items-center gap-2 transition shadow-md hover:shadow-lg">
        ...
        <div class="overflow-x-auto">
            <table id="eventTable" class="w-full text-left border-collapse display nowrap" style="width:100%">
                <thead>
                    <tr>
                        <th
                            class="p-4 border-b border-gray-100 bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            No</th>
                        <th
                            class="p-4 border-b border-gray-100 bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Nama Event</th>
                        <th
                            ...
                            Tanggal</th>
                        <th
                            class="p-4 border-b border-gray-100 bg-gray-50 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Partisipan</th>
                        <th
                            class="p-4 border-b border-gray-100 bg-gray-50 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Tugas / Progress</th>
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="p-4 max-w-xs overflow-hidden">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-lg bg-pink-100 flex-shrink-0 flex items-center justify-center text-[#EC46A4] font-bold">
                                        {{ substr($event->name, 0, 1) }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-semibold text-gray-800 truncate" title="{{ $event->name }}">
                                            {{ $event->name }}</p>
                                        <p class="text-xs text-gray-500 line-clamp-1">
                                            {{ Str::limit(strip_tags($event->description), 30) }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4 text-sm text-gray-500">
                                <div class="flex flex-col">
                                    <span
                                        class="font-medium">{{ $event->event_start ? $event->event_start->format('d M Y') : 'TBA' }}</span>
                                    <span
                                        class="text-xs text-gray-400">{{ $event->event_end ? 's/d ' . $event->event_end->format('d M Y') : '' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="p-4 text-center">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer toggle-status"
                                        data-event-id="{{ $event->id }}" {{ $event->is_active ? 'checked' : '' }}>
                                    <div
                                        class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#EC46A4] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500">
                                    </div>
                                </label>
                            </td>
                            <td class="text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('admin.event.tasks.index', $event->id) }}"
                                        class="text-purple-500 hover:text-purple-700 bg-purple-50 hover:bg-purple-100 p-2 rounded-lg transition"
                                        title="Kelola Tugas">
                                    ...
                                    <button type="button"
                                        class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition delete-event-btn"
                                        data-event-id="{{ $event->id }}" title="Hapus">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                            fill="currentColor">
                                            <path fill-rule="evenodd"
    ...
    <div id="createModalWrapper" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="flex justify-between items-center mb-5">
                        <h3 class="text-xl font-bold text-gray-800" id="modal-title">Tambah Event Baru</h3>
                    ...
                    <form id="createForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Event <span
                                            class="text-[#EC46A4]">*</span></label>
                                    <input type="text" name="name"
                                        class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                        required>
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Maksimal Tim (Kuota)
                                        <span class="text-[#EC46A4]">*</span></label>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Registrasi Mulai
                                            <span class="text-[#EC46A4]">*</span></label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Registrasi Selesai
                                            <span class="text-[#EC46A4]">*</span></label>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Event Mulai <span
                                                class="text-[#EC46A4]">*</span></label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Event Selesai <span
                                                class="text-[#EC46A4]">*</span></label>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Penyisihan
                                            (Opsional)</label>
                                        <div class="space-y-2">
                                            <input type="date" name="preliminary_date"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition">
                                            <select name="preliminary_type"
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Final
                                            (Opsional)</label>
                                        <div class="space-y-2">
                                            <input type="date" name="final_date"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition">
                                            <select name="final_type"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition bg-white">
                                                <option value="">-- Pilih Jenis --</option>
                                                <option value="quiz">Quiz</option>
                                                <option value="project">Project</option>
                                ...
                                <div>
                                    ...
                                    <input type="file" name="photo" accept="image/*" class="dropify"
                                        data-height="150" data-allowed-file-extensions="jpg jpeg png gif"
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi Event <span
                                            class="text-[#EC46A4]">*</span></label>
                        ...
                        <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-gray-100">
                            <button type="button" onclick="closeModals()"
                                class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-lg transition">Batal</button>
                            <button type="submit" id="createBtn"
                                class="px-5 py-2.5 bg-[#EC46A4] hover:bg-[#d63f93] text-white font-semibold rounded-lg shadow-md transition transform active:scale-95">Simpan
                                Event</button>
                        </div>
                    </form>
                </div>
    ...
    <!-- Edit Event Modal Wrapper -->
    <div id="editModalWrapper" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title"
        role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Maksimal Tim (Kuota)
                                        <span class="text-[#EC46A4]">*</span></label>
                                    <input type="number" id="editMaxMembers" name="max_members"
                                        class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                        min="1" required>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Registrasi Mulai
                                            <span class="text-[#EC46A4]">*</span></label>
                                        <input type="date" id="editRegStart" name="registration_start"
                                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                            required>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Registrasi Selesai
                                            <span class="text-[#EC46A4]">*</span></label>
                                        <input type="date" id="editRegEnd" name="registration_end"
                                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                            required>
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Event Mulai <span
                                                class="text-[#EC46A4]">*</span></label>
                                        <input type="date" id="editEventStart" name="event_start"
                                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                            required>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Event Selesai <span
                                                class="text-[#EC46A4]">*</span></label>
                                        <input type="date" id="editEventEnd" name="event_end"
                                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition"
                                            required>
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Penyisihan
                                            (Opsional)</label>
                                        <div class="space-y-2">
                                            <input type="date" id="editPreliminary" name="preliminary_date"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition">
                                            <select id="editPreliminaryType" name="preliminary_type"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition bg-white">
                                                <option value="">-- Pilih Jenis --</option>
                                                <option value="quiz">Quiz</option>
                                                <option value="project">Project</option>
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Final
                                            (Opsional)</label>
                                        <div class="space-y-2">
                                            <input type="date" id="editFinal" name="final_date"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition">
                                            <select id="editFinalType" name="final_type"
                                                class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#EC46A4] outline-none transition bg-white">
                                                <option value="">-- Pilih Jenis --</option>
                                                <option value="quiz">Quiz</option>
                                                <option value="project">Project</option>
                                ...
                                <div>
                                    ...
                                    <input type="file" id="editPhotoInput" name="photo" accept="image/*"
                                        class="dropify" data-height="150" data-allowed-file-extensions="jpg jpeg png gif"
                                        data-max-file-size="5M">
                                    <p class="text-xs text-gray-400 mt-1">Upload baru untuk mengganti. Format: JPG, JPEG,
                                        PNG, GIF. Maks: 5MB.</p>
                                </div>
                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        // Modal Logic
        const createModal = document.getElementById('createModalWrapper');
        const editModal = document.getElementById('editModalWrapper');
